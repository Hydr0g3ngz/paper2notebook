...
Use `scripts/scaffold_notebook.py` when starting a notebook from scratch. Use `scripts/extract_pdf_figure.py` to render a source-grounded crop from a PDF. Run `scripts/validate_notebook.py` during final QA.

## Workflow

1. Inspect the complete source, not only the abstract. Identify the paper type, research task, claimed gap, contribution, decisive evidence, limitations, and prerequisites.
...
4. Design the concept chain before writing code:
...
5. Select source figures by explanatory value. Include architecture/algorithm figures and at least one key evidence figure when available. Crop tightly, preserve labels, record figure number and page, and add guided interpretation. Never use figures as decoration.
...
13. Execute the notebook from a fresh kernel. Fix all errors, broken local images, missing dependencies, implausible outputs, clipped figures, and stale outputs before delivery.
